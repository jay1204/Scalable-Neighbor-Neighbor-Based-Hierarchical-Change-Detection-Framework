...
def main():
  ...
  recordfile = open("recordfile_sample_size.txt",'w')

  leftstr = "s="
  for i in range(1,11):
    recordfile.write("leftstr"+str(i*100)+'\n')
    diff = [0 for x in range(7)]
    filename1 = leftstr+str(i*100)+"/whole_col_p_out1.txt"
    filename2 = leftstr+str(i*100)+"/whole_col_p_out2.txt"
    f1 = open(filename1,"rb")
    f2 = open(filename2,"rb")
    outputfilenamepart = leftstr+str(i*100)+ "/whole_possible_change_map_k_"+str(i)+"_"
    outputMatrix = [[0 for x in range(9600)] for x in range(7)]
    possChangefilename = leftstr+str(i*100)+"/whole_possible_change_map.txt"
    possChangefile = open(possChangefilename,'w')
    
    matchfilename = "hierarchical_clustering_labels_2_8_Euclidean_sample_"+str(i*100)+".txt"
